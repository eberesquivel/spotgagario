...
function setupSocket(socket) {
    ...
    socket.on('welcome', function (playerSettings) {
        ...
        window.chat.addSystemLine('Conectado al juego!');
        window.chat.addSystemLine('çescribe <b>-help</b> para listar los comandos.');
        if (global.mobile) {
    ...
    socket.on('playerDied', function (data) {
        window.chat.addSystemLine('{Jugador} - <b>' + (data.name.length < 1 ? 'Sin nombre' : data.name) + '</b> ha sido comido');
    });

    socket.on('playerDisconnect', function (data) {
        window.chat.addSystemLine('{Jugador} - <b>' + (data.name.length < 1 ? 'Sin nombre' : data.name) + '</b> se ha desconectado.');
    });

    socket.on('playerJoin', function (data) {
        window.chat.addSystemLine('{Jugador} - <b>' + (data.name.length < 1 ? 'Sin nombre' : data.name) + '</b> ha ingresado.');
    });
        ...
        for (var i = 0; i < leaderboard.length; i++) {
            ...
            if (leaderboard[i].id == player.id){
                if(leaderboard[i].name.length !== 0)
                    ...
                    status += '<span class="me">' + (i + 1) + ". Sin nombre</span>";
            } else {
                if(leaderboard[i].name.length !== 0)
                    status += (i + 1) + '. ' + leaderboard[i].name;
                else
                    status += (i + 1) + '. Sin nombre';
            }
        }
    ...
    if (global.died) {
        ...
        if (global.kicked) {
            if (reason !== '') {
              ...
              graph.fillText('Descpmectadp!', global.screenWidth / 2, global.screenHeight / 2);
        }
    }
}
